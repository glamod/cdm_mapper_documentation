...
All CDM fields are read as objects. null values are read with their null value 
int the table files, or else as NaN if na_values argument is set to the null 
value in the file.

Reads the full set of files (default), a subset or a single table, as controlled
by cdm_subset:
    - When reading multiple tables, the resulting dataframe is multiindexed in
...
The full table set (header, observations-*) is assumed to be in the same
 directory.
...
def read_tables( tb_path, tb_id, cdm_subset = None, delimiter = '|', 
    ...
    for k,v in file_patterns.items():
        ...
        if len(file_path) == 1:
            ...
            logger.error('Pattern {0} resulted in multiple files for table {1}. Cannot seccurely retrieve cdm table(s)'.format(tb_id,k))
            return
    
    if len(file_paths) == 0:
        logger.error('No cdm table files found for search patterns {0}: '.format(','.join(list(file_patterns.values()))))
        return
    ...
    if col_subset:
        if len(tables) == 1:
            ...
            if not isinstance(col_subset,dict):
                logger.error('Column subset (col_subset) has to be declared as a dictionary with a table:[columns] pair per table to subset')
                return
            else:
