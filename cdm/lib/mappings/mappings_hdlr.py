...
# this should only matter creating final tables
...
class smart_dict(dict):
    ...
    Returns
    -------
     None: if key or combination not found
    """
        ...
        if self.__depth__ > 1:
            for d in range(2, self.__depth__ + 1):
                self.__getstr__.append(
                    self.__getstr__[d - 2].replace('None', '{}') + '.get(key[' + str(d - 1) + '],None)')
...
def load_code_tables_maps(imodel, codes_subset=None, log_level='INFO'):
    ...
    if codes_subset:
        ...
        if any(not_in_cdm):
            logger.error(
                'A wrong code table was requested for in model {0}: {1}'.format(imodel_codes_lib,
                                                                                ",".join(not_in_cdm)))
...
def load_tables_maps(imodel, cdm_subset=None, log_level='INFO'):
    ...
    if cdm_subset:
        ...
        if any(not_in_cdm):
            logger.error(
                'A wrong cdm table was requested for in model {0}: {1}'.format(imodel_lib, ",".join(not_in_cdm)))
